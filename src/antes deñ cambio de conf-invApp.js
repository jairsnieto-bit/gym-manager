...
const App = () => {
  ...
  const { clients, plans, trainers, machines, inventory, salesInvoices, loading, error, refetch } = useData();
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex space-x-2">
                    <button 
                      onClick={() => {
                        // Obtener categorías y ubicaciones para el formulario
                        const categories = [...new Set(inventory.map(item => item.category))];
                        const locations = [...new Set(inventory.map(item => item.location))];
                        setInventoryForm({
                          name: '',
                          category: categories[0] || 'Supplements',
                          stock: 0,
                          price: 0,
                          min_stock: 5,
                          supplier: '',
