...
import { Calendar, Users, Dumbbell, Package, CreditCard, Settings, FileText, User, ShoppingCart } from 'lucide-react';
...
const Sidebar = ({ activeTab, setActiveTab, userRole = 'admin' }) => {
  ...
  // Función para verificar si una pestaña está activa
  const isTabActive = (tabId) => {
    if (tabId === 'inventory-config') {
      return activeTab === 'inventory-config' || 
             activeTab === 'inventory-categories' || 
             activeTab === 'inventory-locations' || 
             activeTab === 'inventory-units';
    }
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-3 py-2 text-sm font-medium rounded-md ${
                  isTabActive(tab.id) ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'
                }`}
              >
